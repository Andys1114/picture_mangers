...
- **`post_count` maintained at write time** — `Tag.post_count` is bumped (±1) by whatever service mutates `post_tags` (`tag_post`, `create_implication` backfill). It always equals the number of `post_tags` rows for that tag; no lazy recompute. This slice does not delete tags or implications, so there is no decrement path yet (ADR-0001 "sticky delete").
