...
const CHIP =
  "inline-flex cursor-pointer items-center gap-[5px] rounded-pill border px-[11px] py-1 font-mono text-xs font-medium transition duration-150 ease-out-soft hover:brightness-125 active:scale-[0.97] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring";

/** 已选条件 chips 区：URL 里的标签 + 评级子集，逐个可删（点击即从 URL 移除）。
 *  标签分类色从标签索引 + 连带树缓存（与 rail-tags 同一份查询）反查，
 *  查不到按 general 展示。无选中时整块不渲染。 */
export default function RailChips() {
